import { getLorePosts, type LorePost } from './utils'
 ...
 */
function filterByFrom(posts: LorePost[], from?: string): LorePost[] {
  ...
  // FROM #tag (reserved for future tag support)
  const tag = from.substring(1)
  return posts.filter(post => {
    const tags = (post.metadata as Record<string, unknown>)['tags']
 ...
 */
function filterByWhere(posts: LorePost[], where?: string): LorePost[] {
...
export function executeDataviewQuery(query: string): string {
  ...
  let posts: LorePost[] = getLorePosts()
